...
class PlayerManager:
    ...
    def play(self, url: str, title: str, player_type: str = 'mpv', subtitle_urls: Optional[list[str]] = None):
        api.log_debug(f"DEBUG: PlayerManager.play - Received URL: {url}")
        try:
            if player_type == 'vlc':
                self._play_vlc(url, title)
            else:
                self._play_mpv(url, title, subtitle_urls)
    ...
    def _play_mpv(self, url: str, title: str, subtitle_urls: Optional[list[str]] = None):
        ...
        # Add force-window to ensure window appears even if audio-only (rare for mov but good safety)

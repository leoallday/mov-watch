...
def get_pypi_latest_version():
    try:
        resp = requests.get('https://pypi.org/pypi/mov-watch/json', timeout=10)
        if resp.status_code == 200:
            data = resp.json()
        ...
        if latest > current:
            ...
            try:
                result = subprocess.run(
                    [sys.executable, '-m', 'pip', 'install', '--upgrade', 'mov-watch'],
                    capture_output=True,
                    text=True
                )
                ...
                if result.returncode == 0:
                    ...
                    if 'mov-watch' in str(sys.argv[0]).lower():
                        cmd = 'mov-watch'
                    else:
                        cmd = 'mov-watch'
                    ...
                    _print_info("Please try manually: pip install --upgrade mov-watch")
                    print()
...
def check_executable_update():
    ...
    try:
        ...
        if latest > current:
            ...
            _print_info("  - Pip: pip install mov-watch")
            _print_info("  - AUR: yay -S mov-watch")
            print()
            input("Press ENTER to continue...")
            return False
